# 50 Proven Short-Form Post Formats

Use these formats exactly as structural skeletons. Fill in every bracketed placeholder with real,
specific content based on the user's topic, audience, and (where relevant) chosen content pillar /
audience archetype / sub-topic. Never leave template brackets in the final output — every post must
be fully written and ready to publish.

### Format #1: Universal Feelings
What I thought would make me [Feeling]:

[Achievement1]
[Achievement2]
[Achievement3]

What actually makes me happy:

[SimplePleasure1]
[SimplePleasure2]
[SimplePleasure3]

Everyone chases the 1st list—but it usually ends in disappointment.

### Format #2: Getting Unstuck
Feeling stuck? [SimpleSolution]

No [Distraction1]
No [Distraction2]
No [Distraction3]

Just you, your [PersonalTrait1], and your [PersonalTrait2].

It works every time.

### Format #3: "Hacks"
[X] "hacks" for better [ReachTopic]:

[SimpleAdvice1]
[SimpleAdvice2]
[SimpleAdvice3]
[SimpleAdvice4]
[SimpleAdvice5]

Turns out, the only real [ReachTopic] hacks aren't really hacks at all.

### Format #4: Life Transformation
Me at [Age]

[Negative1]
[Negative2]
[Negative3]

Me at [Age+]

[Positive1]
[Positive2]
[Positive3]

[X] years can change everything.

Don't give up.

### Format #5: Easy/Hard Life Advice
If you are [Age] and you want [Outcome], stop screwing around.

Choose [HardDecision] over [EasyDecision]
Choose [HardDecision] over [EasyDecision]
Choose [HardDecision] over [EasyDecision]
Choose [HardDecision] over [EasyDecision]

In the short-term, you'll look crazy.

In the long term, you'll be wildly successful.

### Format #6: Must-Have Skills
[X] must-have [PopularPlatform/Topic] skills everyone should know:

Skill #1
Skill #2
Skill #3

### Format #7: Don't Waste Time
There are a zillion ways to waste time online.

(Everyone knows how to do that.)

But if you want to save time and accomplish 10x more, here are [X] [Tools] that will make your
Internet time more productive:

### Format #8: Lies You've Been Told
[X] lies you've been told about [Topic]:

Lie #1
Lie #2
Lie #3

### Format #9: Timeless Wisdom
I am obsessed with learning about [ReachTopic].

So I went and devoured [X] years of [FamousPerson] advice.

The result?

I walked away with [X] timeless principles from the greatest [FamousPersonStatus] in the world.

Here are the golden takeaways:

### Format #10: Contrarian Views
I used to see the world the same as the 99%.

But since adopting these [X] contrarian views held by the top 1%, how I view [ReachTopic] has
completely changed (and got me to [GiantOutcome]):

### Format #11: Brainstorming Hack
Today I started brainstorming a new idea while I was [UnconventionalActivity].

[StepAlreadyDone1]
[StepAlreadyDone2]
[StepAlreadyDone3]

Takeaway: [UnconventionalActivity] is a [Niche] hack.

### Format #12: People Think
People think starting [Niche] is:

[ConventionalWisdom1]
[ConventionalWisdom2]
[ConventionalWisdom3]

What it actually is:

[UnconventionalWisdom1]
[UnconventionalWisdom2]
[UnconventionalWisdom3]

That's literally it.

### Format #13: Anyone Can
You can accomplish [NicheGoal] if you…

[SimpleAction1]
[SimpleAction2]
[SimpleAction3]
[SimpleAction4]

Congrats.

You [GoalAccomplished].

### Format #14: Simple Ways
Ways [Niche] makes your life better:

[Outcome1]
[Outcome2]
[Outcome3]
[Outcome4]
[Outcome5]

Every person should [NicheAction].

### Format #15: Unexpected Ways
Unexpected ways [Niche] makes your life better:

[UnexpectedOutcome1]
[UnexpectedOutcome2]
[UnexpectedOutcome3]
[UnexpectedOutcome4]
[UnexpectedOutcome5]

Every person should [NicheAction].

### Format #16: Niche Tip
[Niche] Tip: [Tip1]

Don't [WrongAdvice].

Instead, [RightAdvice].

This ensures [DesirableOutcome].

### Format #17: Best Tools
Best [Niche] tools in [Year]:

[ToolName]: [SentenceDescription]
[ToolName]: [SentenceDescription]
[ToolName]: [SentenceDescription]
[ToolName]: [SentenceDescription]
[ToolName]: [SentenceDescription]

### Format #18: Individual Book Breakdown
[BookMainTitle]

[BookSubtitle]

Back in [Year], legendary [StatusTitle] [Author] distilled his [X] years of wisdom into [X] pages.

And in this book, [He/She] shared a simple [X]-step process for [Outcome].

Here's the breakdown:

### Format #19: Curated Blog Posts
2.5 billion blog posts are published each year.

But it's impossible to read that much content.

So, ignore the 2,499,999,992 that aren't worth your time.

And just read these 8:

### Format #20: The Trick
Most [Topic People] focus on [Wrong Action].

This gets them nowhere.

Instead, the trick is to focus 100% of your attention on [Right Action]:

[Step #1]
[Step #2]
[Step #3]

### Format #21: 10 Years Of Practice
They say it takes 10 years to master a craft.

But using this framework, I mastered [Topic] in 1/10th the time.

[Step #1]
[Step #2]
[Step #3]
[Step #4]

Here's how it works (so you can too):

### Format #22: 30 Second Explanation
Most people think learning [Topic] takes 10,000 hours.

But I can explain it to you in 30 seconds.

A quick breakdown on:

[Main Point #1]
[Main Point #2]
[Main Point #3]

Let's dive in…

### Format #23: 90% Of Outcomes
90% of [Outcome] is really just:

[Simple Habit #1]
[Simple Habit #2]
[Simple Habit #3]

The little stuff that makes all the difference.

### Format #24: Amateurs vs Pros
How amateurs [Topic]

[Amateur Approach]
[Amateur Approach]
[Amateur Approach]

How the pros do it

[Pro Approach]
[Pro Approach]
[Pro Approach]

### Format #25: Another Benefit
Another benefit of [Topic]:

[Benefit]

[Reason Why]

### Format #26: Old Life vs New Life
When I [OldLifeCompleted], I [NewLifeBeginning].

It was [NegativeDetail].

[NegativeDetail]. [NegativeDetail].

[X] years later, I just [LifeAchievement].

Here's what those difficult years taught me about [Topic]:

### Format #27: Tools For Success
Anyone with an Internet connection can start an $XXK online business with nothing but a laptop:

[Tool1] is free.
[Tool2] is free.
[Tool3] is free.
[Tool4] is free.
[Tool5] is free.
[Tool6] is free.
[Tool7] is free.

No more excuses.

### Format #28: Author Advice
Writing advice from best-selling author, [Famous Author].

Stop [Writing Mistake].

Start [Writing Tip].

### Format #29: Biggest Mistakes
The [X] biggest mistakes [Topic People] make:

[Mistake #1]
[Mistake #2]
[Mistake #3]

Let's fix them:

[Easy Solution #1]
[Easy Solution #2]
[Easy Solution #3]

### Format #30: Books Worth Reading
Over the past [X] years, I have read over [X] [Topic] books.

99% were 1 idea stretched across 300 pages.

They should have been blog posts.

Save yourself the time and just read these [X]:

### Format #31: Best Books On Topic
The best books I've ever read on [Topic]:

[Book #1]
[Book #2]
[Book #3]

All this knowledge for less than $100!

### Format #32: Creativity On A Niche Topic
Being creative is draining.

Here's my cheat sheet so you'll never run out of [Topic] inspiration.

[X] easy ways to generate quality ideas:

### Format #33: Overcome These Challenges
[X] challenges you will experience when trying to [Topic]:

[Challenge #1]
[Challenge #2]
[Challenge #3]

But, overcome these and you win.

### Format #34: Best Companies
The best [Topic] companies of all time:

[Company #1]
[Company #2]
[Company #3]

Everyone should study their origin stories.

### Format #35: Another Framework
Another great framework for solving [Topic Problem] quickly:

[Step #1]
[Step #2]
[Step #3]
[Step #4]

Works every time.

### Format #36: Creators To Follow
Creators you should follow if you're interested in [Topic]:

[Creator #1]
[Creator #2]
[Creator #3]
[Creator #4]
[Creator #5]

They will teach you more than any professor.

### Format #37: Fascinating Data
Fascinating [Year] data on [Topic]:

[Data Point #1]
[Data Point #2]
[Data Point #3]
[Data Point #4]

Signals of where the world is headed.

### Format #38: I Wake Up Ecstatic
Every single day, I wake up ecstatic to do 5 things:

1. [Activity1]
2. [Activity2]
3. [Activity3]
4. [Activity4]
5. [Activity5]

Each for different reasons:

### Format #39: Masterclass Synopsis
[FamousPerson] is one of the [Credibility] of all time.

[FamousOutcome1]
[FamousOutcome2]
[FamousOutcome3]

I am fascinated by [his/her] career.

So I watched [his/her] MasterClass (twice!).

Here are the golden nuggets:

### Format #40: The Godfather
The Godfather of [Industry]: [Name]

In [Year], [he/she] published [BookName], a book so good the original [RecordOutcome].

And inside, [he/she] shared the [Topic] routine that helped [him/her] [Outcome1] and [Outcome2].

Here's the breakdown:

### Format #41: The King Of [Platform]
The KING of [Platform]:

[Name]

Over the last 12 months, [He/She] has gained over [X] subscribers.

So I binge watched [X] hours of [His/Her] videos to study how [He/She/They] have grown so quickly.

And here's [His/Her] [X]-part framework for rapid growth (that you can use on any platform):

### Format #42: Topic 101
[Topic] 101

Over the past [X] years, I have [Credibility1], [Credibility2], [Credibility3].

Want to know a secret?

I use the same [X] templates every time:

### Format #43: Zero To Hero (In Your Niche)
[X] years ago, I didn't know anything about [Niche].

Today, I am [Achievement].

Here are [X] unforgettable lessons I've learned about [Topic]:

### Format #44: The Bestselling Book Framework
[Book] is the [Number] best-selling book of the [Time Period].

Only behind [Another Well-Known Book]

And [Author] used [Framework] for [Surprising Use]

Here's the breakdown:

### Format #45: My Favorite Framework
My favorite [Topic] framework:

[Step #1]
[Step #2]
[Step #3]
[Step #4]
[Step #5]

Works every time.

### Format #46: Reach The 1%
[Topic] goals you should strive for:

[Goal #1]
[Goal #2]
[Goal #3]
[Goal #4]

Achieving these puts you in the top 1%.

### Format #47: Wild Stories
[Person/Company] has officially reached [GiantOutcome].

Here's what you probably don't know:

When [Person/Company] first started back in [Date], they had [NoOutcome].

But just a few [Time] later, they [FirstBigOutcome].

How did this happen? [Twist]

Here's the story:

### Format #48: Niche Tip (Good vs Bad)
Good [Topic]

[Good Trait]
[Good Trait]
[Good Trait]

Bad [Topic]

[Bad Trait]
[Bad Trait]
[Bad Trait]

### Format #49: 1,000 Hours
I spent over 1,000 hours learning to [Skill] effectively — because college completely failed to
teach me.

Here's everything I learned distilled into [X] posts (that you can start applying today):

### Format #50: The Quick Fixes
Are you struggling with [Problem]?

I have spent the past [X] [Time] studying [Problem] relentlessly.

And what I've found is that it can be easily solved with [X] quick fixes:
